                ...
                <a class="nav-link nav-icon" href="#" data-bs-toggle="dropdown">
                    ...
                    @if (count($newOrders) > 0 || count($pendingOrders) > 0)
                        <span id="sound_alarm" class="badge-number alarm_count">
                            <span class="spinner-grow spinner-grow-sm bg-danger" role="status"
                ...
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow messages">
                    @if (count($newOrders) > 0)
                        <li class="dropdown-header">
                            You have <span class="alarm_count">{{ count($newOrders) }}</span> <span
                                class="badge rounded-pill bg-danger">New</span> Orders
                            <a href="{{ route('allOrders', 'new') }}"><span
                    ...
                    @if (count($pendingOrders) > 0)
                        <li class="dropdown-header">
                            You have <span class="alarm_count">{{ count($pendingOrders) }}</span> <span
                                class="badge rounded-pill bg-success">Pending</span> Orders
                            <a href="{{ route('allOrders', 'pending') }}"><span
                                    class="badge rounded-pill bg-primary p-2 ms-2">View all</span></a>
                        </li>
                        <li>
                            <hr class="dropdown-divider" />
                    ...
                    @if (count($newOrders) > 0)
                        @foreach ($newOrders as $newOrder)
                            <li class="message-item">
                                <div>
                                    <h4>Order: {{ $newOrder->orderCode($newOrder->id) }} &nbsp;
                                        <span>{{ ucFirst($newOrder->status) }}</span>
                                    </h4>
                                    <h6>Customer:
                                        {{ $newOrder->customer->firstname . ' ' . $newOrder->customer->lastname }}
                                    </h6>
                                    <p class="text-dark">Due Date:
                                        {{ \Carbon\Carbon::parse($newOrder->expected_delivery_date)->format('D, jS M Y') }}
                                    </p>
                    ...
                    @if (count($pendingOrders) > 0)
                        @foreach ($pendingOrders as $pendingOrder)
                            <li class="message-item">
                                <div>
                                    <h4>Order: {{ $pendingOrder->orderCode($pendingOrder->id) }} &nbsp;
                                        <span>{{ ucFirst($pendingOrder->status) }}</span>
                                    </h4>
                                    <h6>Customer:
                                        {{ $pendingOrder->customer->firstname . ' ' . $pendingOrder->customer->lastname }}
                                    </h6>
                                    <p class="text-dark">Due Date:
                                        {{ \Carbon\Carbon::parse($pendingOrder->expected_delivery_date)->format('D, jS M Y') }}
                                    </p>
            ...
            <li class="nav-item dropdown pe-3">
                <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
                    ...
                    @if (isset(Auth::user()->profile_picture))
                        <img src="{{ asset('/storage/staff/' . Auth::user()->profile_picture) }}" alt="Profile"
