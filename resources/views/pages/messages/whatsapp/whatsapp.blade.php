...
<main id="main" class="main">
  ...
  <div class="pagetitle">
    <h1>Whatsapp Sent Messages: @if($order !== "") <span class="text-dark">{{ $order->customer->firstname.' '.$order->customer->lastname }}</span> @endif</h1>
    <nav>
      <ol class="breadcrumb">
              ...
              <tbody>
                @if (count($messages) > 0)
                    @foreach ($messages as $message)
                        ...
                        <tr>
                            <td data-categoryname="{{ isset($message->to) ? $message->to : '' }}" class="categoryname">{{ $message->topic }}</td>
                            ...
                            <td>
                              @if (isset($message->to) && $message->to=='employees')
                                ...
                                <span class="badge badge-dark mr-1">{{ $user->phone_1 }}</span>
                                @endforeach
                              @endif

                              @if (isset($message->to) && $message->to=='customers')
                                @foreach ($customers as $customer)
                                <span class="badge badge-dark mr-1">{{ $customer->whatsapp_phone_number }}</span>
                                @endforeach
                              @endif
                            ...
                            <td>{{ substr($message->message, 0, 30) . '...' }} <br>
                              <span class="badge badge-dark" onclick="editSentMailModal('{{ $message->id }}', '{{ $message->topic }}', '{{ $message->message }}')" style="cursor: pointer;">view more</span>
                            </td>
