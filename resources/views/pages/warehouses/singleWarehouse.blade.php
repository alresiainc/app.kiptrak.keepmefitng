          ...
          <div class="card">
            ...
            <div class="card-body pt-3">
              ...
              <div class="row">
                ...
                <div class="row g-3 shadow bg-white rounded">
                  <div class="col-lg-3">
                    <label for="">Code</label>
            ...
            <div class="card-body pt-3">
              ...
              <div class="row g-3 m-1">
                <div class="col-lg-3">
                    ...
                    <div class="text-dark display-6">{{ $package['order']->orderCode($package['order']->id) }}</div>
                    <div class="float-lg-start">
                      <a href="{{ route('singleOrder', $package['order']->unique_key) }}"><button class="btn btn-sm btn-success"><i class="bi bi-eye"></i></button></a>
                    </div>
                </div>
                <div class="col-lg-5">
                    <label for="" class="fw-bolder">Customer</label>
                    <div class="text-dark">{{ $package['order']->customer_id ? $package['order']->customer->firstname : 'N/A' }} 
                      {{ $package['order']->customer_id ? $package['order']->customer->lastname : 'N/A' }}
                        | Email: <span class="lead">{{ $package['order']->customer_id ? $package['order']->customer->email : 'N/A' }}</div>
                    <div>Phone:  <span class="lead">{{ $package['order']->customer_id ? $package['order']->customer->phone_number : 'N/A' }}</span><br>
                        @if ($package['order']->customer_id)
                    
                        @php
                            $whatsapp = substr($package['order']->customer->whatsapp_phone_number, 1)
                        @endphp
                        Whatsapp:  <span class="lead"><a href="https://wa.me/{{ '234'.$whatsapp }}?text=Hi" target="_blank">
                            {{ $package['order']->customer->whatsapp_phone_number }}</a></span>
                        @else
                            Whatsapp:  <span class="lead">None</span>
                        @endif
                        {{-- <a href="https://wa.me/2348066216874?text=Hi">Whatsapp link</a> --}}
                    </div>
                    <div>Location:  <span class="lead">{{ $package['order']->customer_id ? $package['order']->customer->city : 'None' }}, {{ $package['order']->customer_id ? $package['order']->customer->state : 'None' }}</span></div>
                    <div>Delivery Address:  <span class="lead">{{ $package['order']->customer_id ? $package['order']->customer->delivery_address : 'None' }}</span></div>
                    
                </div>
                <div class="col-lg-2">
                    <label for="" class="fw-bolder">Expected Revenue({{ $currency }})</label>
                    <div class="text-dark display-6">{{ $package['orderRevenue'] }}</div>
                </div>
                <div class="col-lg-2">
                    <label for="" class="fw-bolder">Agent</label>
                    <div class="text-dark">{{ $package['order']->agent_assigned_id ? $package['order']->agent->name : 'None' }}</div>
                </div>
              </div>

              <hr>
              
              @foreach ($package['outgoingStockPackageBundle'] as $product_bundle)
              
              <div class="row g-3 m-1">
                
                <div class="col-lg-6">
                    <label for="" class="fw-bolder">Product Name</label>
                    <div class="text-dark" style="font-size: 14px;">{{ $product_bundle['product']->name }}</div>
                </div>

                <div class="col-lg-3">
                    <label for="" class="fw-bolder">Quantity Ordered</label>
                    <div class="text-dark d-none" style="font-size: 14px;">{{ $product_bundle['quantity_removed'] }} @ {{ $product_bundle['product']->sale_price  }} </div>
                    <div class="text-dark" style="font-size: 14px;">{{ $product_bundle['quantity_removed'] }} @ {{ $product_bundle['product']->sale_price  }}</div>
                </div>
                
                <div class="col-lg-3">
                    <label for="" class="fw-bolder">Revenue</label>
                    <div class="text-dark" style="font-size: 14px;">{{ $product_bundle['amount_accrued'] }}</div>
                </div>
