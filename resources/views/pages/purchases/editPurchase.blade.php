...
@section('title')Edit Purchase @endsection
@section('extra_css')
    <style>
        select{
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <form class="row g-3 needs-validation" action="{{ route('editPurchasePost', $purchase->unique_key) }}" method="POST"
              ...
              <div class="col-md-12 mb-3">The field labels marked with * are required input fields.</div>
                <div class="col-md-6">
                  ...
                  <input type="text" name="purchase_code" class="form-control @error('purchase_code') is-invalid @enderror" value="{{ $purchase->purchase_code }}" readonly>
                  @error('purchase_code')
                    ...
                    <table id="orderTable" class="table caption-top">
                        ...
                        <tbody>
                            @foreach ($purchases as $purchase)
                                <tr>
                                    <input type='hidden' name='incoming_stock_id[]' value='{{ $purchase->incoming_stock_id }}'>
                                    <th scope='row'>{{ $purchase->product->name }}</th>
                                    <td><input type='hidden' name='product_id[]' value='{{ $purchase->product->id }}'>
                                        {{ $purchase->product->code }}</td>
                ...
                <div class="col-md-4">
                    ...
                    <select name="payment_type" id="payment_type" data-live-search="true" class="custom-select form-control border @error('payment_type') is-invalid @enderror" id="">
                      <option value="{{ $purchase->payment_type }}">{{ $purchase->payment_type }}</option>
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="note" id="" name="note" class="form-control @error('note') is-invalid @enderror" cols="30" rows="10">{{ isset($purchase->note) ? $purchase->note : ''}}</textarea>
