                                                ...
                                                <tr>
                                                    ...
                                                    @if (isset($formHolder->staff_assigned_id))
                                                        <td>
                                                            ...
                                                            <span class="badge badge-dark"
                                                                onclick="changeAgentModal('{{ $formHolder->id }}')"
                                                    ...
                                                    @else
                                                        <td style="width: 120px">
                                                            <span class="badge badge-success"
                                                                onclick="addAgentModal('{{ $formHolder->id }}')"
                                                                data-bs-toggle="tooltip" data-bs-placement="left"
                                                    ...
                                                    @if (!isset($formHolder->order->customer_id))
                                                        @if (isset($formHolder->orderbump_id) && isset($formHolder->orderbump->product->id))
                                                            <td>
                                                                <a href="{{ asset('/storage/products/' . $formHolder->orderbump->product->image) }}"
                                                                    ...
                                                                    <img src="{{ asset('/storage/products/' . $formHolder->orderbump->product->image) }}"
                                                                        width="30" class="img-thumbnail img-fluid"
                                                                        alt="{{ $formHolder->orderbump->product->name }}"
                                                                        style="height: 30px;"></a>

                                                                <br>
                                                                <span class="badge badge-info editOrderbumpSpan"
                                                                    onclick="editOrderbump({{ json_encode($formHolder) }}, {{ json_encode($formHolder->orderbump->orderbump_subheading) }})"
                                                        ...
                                                        @else
                                                            <td><span class="badge badge-primary"
                                                                    onclick="addOrderbump('{{ $formHolder->unique_key }}', '{{ $formHolder->name }}')"
                                                                    style="cursor: pointer;">
                                                    ...
                                                    @if (!isset($formHolder->order->customer_id))
                                                        @if (isset($formHolder->upsell_id) && isset($formHolder->upsell->product->id))
                                                            <td>
                                                                <a href="{{ asset('/storage/products/' . $formHolder->upsell->product->image) }}"
                                                                    data-fancybox="gallery"
                                                                ...
                                                                <span class="badge badge-info"
                                                                    onclick="editUpsell({{ json_encode($formHolder) }}, '{{ htmlspecialchars($formHolder->upsell->upsell_subheading) }}')"
                                                                    style="cursor: pointer;">
                                                                    <i class="bi bi-pencil"></i> Edit</span>
                                                        ...
                                                        @else
                                                            <td><span class="badge badge-primary"
                                                                    onclick="addUpsell('{{ $formHolder->unique_key }}', '{{ $formHolder->name }}')"
                                                                    style="cursor: pointer;">
                                                    ...
                                                    @if (isset($formHolder->thankyou_id))
                                                        <td>
                                                            {{ $formHolder->thankyou->template_name }} <span
                                                                class="badge badge-info"
                                                                onclick="addThankYouTemplate('{{ $formHolder->unique_key }}', '{{ $formHolder->name }}', {{ json_encode($formHolder->thankyou) }})"
                                                                style="cursor: pointer;">
                                                                <i class="bi bi-pencil"></i> Edit</span>
                                                            <br>
                                                    ...
                                                    @else
                                                        <td><span class="badge badge-primary"
                                                                onclick="addThankYouTemplate('{{ $formHolder->unique_key }}', '{{ $formHolder->name }}', '')"
                                                                style="cursor: pointer;">
                                                                <i class="bi bi-plus"></i> Add</span></td>
                                                    @endif

                                                    <td class="d-none">
                                                        <span>{{ isset($formHolder->order->customer_id) ? $formHolder->order->customer->firstname . ' ' . $formHolder->order->customer->lastname : 'No response' }}
                                                        </span>
                                                    </td>
            ...
            <div class="modal-content bg-white">
                ...
                <form action="{{ route('addOrderbumpToForm') }}" method="POST">@csrf
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" id="form_unique_key" class="form_unique_key" name="form_unique_key"
                            value="">

                        <div class="mt-3">
                            <label for="" class="form-label">Heading | Optional</label>
                            <input type="text" id="orderbump_heading" name="orderbump_heading" class="form-control"
                                value="">
                        </div>

                        <div class="mt-3 d-none">
                            <label for="" class="form-label">Sub Heading | Optional</label>
                            <input type="text" id="orderbump_subheading2" name="orderbump_subheading"
                            ...
                            <div class="product-clone-section wrapper">
                                <div class="col-md-12 mt-1 element">
                                    ...
                                    <input type="text" name="orderbump_subheading[]" class="form-control"
                                        placeholder="" value="It is an Amazing Offer">
                                </div>
                        ...
                        <div class="mt-3">
                            ...
                            <select id="orderbump_product" name="orderbump_product" data-live-search="true"
                                class="custom-select form-control border btn-dark @error('orderbump_product') is-invalid @enderror"
            ...
            <div class="modal-content bg-white">
                ...
                <form action="{{ route('editOrderbumpToForm') }}" method="POST">@csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mt-3">
                            <label for="" class="form-label">Heading | Optional</label>
                            <input type="text" name="orderbump_heading" id="editOrderbump_heading"
                ...
                <input type="text" class="form-control" value="lorem color">
                        ...
                        <div class="mt-3">
                            <div class="product-clone-section wrapper2">
                                ...
                                <div class="buttons d-flex justify-content-between">
                                    <button type="button" class="clone2 btn btn-success btn-sm rounded-pill"><i
                                            class="bi bi-plus"></i></button>
                                    <button type="button" class="remove2 btn btn-danger btn-sm rounded-pill"><i
                                            class="bi bi-dash"></i></button>
                                </div>
                        ...
                        <div class="mt-3">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="product_actual_selling_price"
                                        id="productActualSellingPrice" class="form-control" placeholder="" readonly
                                        value="">
                                </div>
                                <div class="col-md-6">
                                    <label for="" class="form-label">Assumed Sale Price</label>
                                    <input type="number" name="product_assumed_selling_price"
                                        id="productAssumedSellingPrice" class="form-control" placeholder=""
                                        value="">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3" id="orderbumpProductSelectWrapper">
                            ...
                            <select id="orderbumpProductSelect" name="orderbump_product" data-live-search="true"
                                ...
                                @if (count($products) > 0)
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" class="selected_option">{{ $product->name }}
                                            @<span class="product_sale_price">{{ $product->sale_price }}</span></option>
                                    @endforeach
                                @endif
            ...
            <div class="modal-content bg-white">
                ...
                <form action="{{ route('addUpsellToForm') }}" method="POST">@csrf
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="addUpsell_form_unique_key" id="addUpsell_form_unique_key"
                            class="addUpsell_form_unique_key" value="">
                        ...
                        <div class="mt-3">
                            ...
                            <select name="upsell_setting_id" data-live-search="true"
                                class="custom-select form-control border btn-dark @error('product') is-invalid @enderror"
                                id="" style="color: black !important;">
                                <option value="">Nothing Selected</option>
                        ...
                        <div class="mt-3">
                            ...
                            <select name="upsell_product" data-live-search="true"
                                class="custom-select form-control border btn-dark @error('product') is-invalid @enderror"
                                id="" style="color: black !important;">
                                <option value="">Nothing Selected</option>
                                @if (count($products) > 0)
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mt-3">
                            ...
                            <select name="upsell_setting_id" id="upsell_setting_id" data-live-search="true"
                                class="custom-select form-control border btn-dark @error('upsell_product') is-invalid @enderror"
                                id="" style="color: black !important;">

                                @if (isset($formHolder->upsell_id) && isset($formHolder->upsell->template->id))
                                    <option value="{{ $formHolder->upsell->template->id }}">
                                        {{ $formHolder->upsell->template->template_code }}</option>
                                @endif
                        ...
                        <div class="mt-3">
                            ...
                            <select name="upsell_product" id="upsellProductSelect" data-live-search="true"
                                ...
                                @if (isset($formHolder->upsell_id) && isset($formHolder->upsell->product->id))
                                    <option value="{{ $formHolder->upsell->product->id }}">
            ...
            <div class="modal-content bg-white">
                ...
                <form action="{{ route('addThankYouTemplateToForm') }}" method="POST">@csrf
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="addThankYou_form_unique_key" id="addThankYou_form_unique_key"
                            class="addThankYou_form_unique_key" value="">

                        <div class="mt-3">
                            <label for="upsell_product" class="form-label">Select Template</label>
                            <select name="thankyou_template_id" data-live-search="true"
                                class="custom-select form-control border btn-dark @error('thankyou_template_id') is-invalid @enderror"
                        ...
                        <div class="mt-3">
                            ...
                            <input type="text" name="template_external_url" id="template_external_url"
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('assignStaffToForm') }}" method="POST">@csrf
                    <div class="modal-body">
                        ...
                        <div class="d-grid mb-3">
                            <label for="">Select Staff</label>
                            <select name="staff_id" id="changeAgentModalSelect" data-live-search="true"
                                class="custom-select form-control border border-dark">
...
@section('extra_js')
    ...
    <script>
        ...
        function editOrderbump($formHolder = "", $orderbump_subheading = "") {
            ...
            $select.prepend('<option value="' + $formHolder.orderbump.product.id + '">' + $formHolder.orderbump.product
                .name + '</option>');

            // Update the custom-select UI
            $select.closest('.bootstrap-select').find('.filter-option-inner-inner').html($formHolder.orderbump.product
                .name);
            $select.closest('.bootstrap-select').find('.dropdown-toggle .filter-option-inner-inner').html($formHolder
            ...
            $.each(options, function(index) {
                $("#myList").append($(
                    "<div class='col-md-12'><input type='text' name='orderbump_subheading[]' class='form-control mb-2' value='" +
                    options[index] + "'></div>"));
            });
        ...
        function editUpsell($formHolder = "", $upsell_subheading = "") {
            ...
            // Prepend the new option
            $select.prepend('<option value="' + $formHolder.upsell.product.id + '">' + $formHolder.upsell.product.name +
                '</option>');

            // Update the custom-select UI
            $select.closest('.bootstrap-select').find('.filter-option-inner-inner').html($formHolder.upsell.product.name);
            $select.closest('.bootstrap-select').find('.dropdown-toggle .filter-option-inner-inner').html($formHolder.upsell
                .product.name);
        ...
        function addThankYouTemplate($form_unique_key = "", $form_name = "", $thankyou = "") {
            ...
            if ($thankyou == "") {
                ...
                $select.closest('.bootstrap-select').find('.dropdown-toggle .filter-option-inner-inner').html($thankyou
                    .template_name);
