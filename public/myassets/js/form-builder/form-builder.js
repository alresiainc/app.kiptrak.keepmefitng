...
$(document).ready(function () {
    ...
    window.addDefaultItems = function (items, reset = true, position = null) {
        ...
        items.forEach((item) => {
            const { type, config } = item; // Destructure type and config from item
            const newItem = $("<div>")
                .addClass("canvas-element")
                .data("type", type);
            $(".drop-container").append(newItem);
            addElementToCanvas(type, newItem, item); // Call function to add element to canvas
        });
        ...
        $("#column-size").on("change", function () {
            if (type == "product") {
                element
                    ...
                    ?.removeClass(
                        "col-sm-12 col-sm-10 col-sm-9 col-sm-8 col-sm-6 col-sm-4 col-sm-3 col-sm-2 col-sm-1"
                    )
                    .addClass($(this).val());
    ...
    function updateConfig(element, type, current) {
        ...
        if (config.label) {
            config.name =
                config.label?.replace(/\s+/g, "_").toLowerCase() || "";
        }
...
`);
        ...
        function reinitializeTinyMCE() {
            ...
            tinymce.init({
                ...
                toolbar:
                    "undo redo | styleselect | fontsize | bold italic | alignleft aligncenter alignright | code",
                fontsize_formats: "8pt 10pt 12pt 14pt 16pt 18pt 24pt 36pt", // Specify font sizes
                toolbar:
                    "undo redo | styleselect | fontsize | bold italic | alignleft aligncenter alignright | bullist numlist outdent indent | link image | code",
        ...
        ell.append(`
            <label class="propertiy-choice">Package Choice:</label>
                ...
                <option value="package_multi" ${
                    config.package_choice == "package_multi" ? "selected" : ""
