(function() {
  ...
  for (let i = 0; i < currency.length; i++) {
    currency[i].innerHTML = curr!=undefined?curr:'₦';
  }
...
// ------------------------------------------------------ //
  ...
  const toggleFullscreen = (elem) => {
    ...
    if (!document.fullscreenElement && !document.mozFullScreenElement && !document.webkitFullscreenElement && !document.msFullscreenElement) {
        ...
        $(".nav-link-expand")
        .find("i")
        .toggleClass("bi-fullscreen-exit bi-fullscreen")
    }
    else {
  ...
  const submitForm = (formId)=> {
    document.forms[formId].addEventListener('submit', (event) => {
      ...
      fetch(event.target.action, {
          ...
          var res = response.json(); // or response.text() or whatever the server sends
          alert('response success: '+JSON.stringify(res));
